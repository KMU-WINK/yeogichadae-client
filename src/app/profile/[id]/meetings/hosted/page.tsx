...
export default function HostedMeetingsPage() {
  const { id } = useParams();
  ...
  const meetings = user.meetings.hosted;
  ...
  return (
    <div className="container mx-auto max-w-screen-xl px-4 py-4 sm:px-6 sm:py-10 md:px-8">
      ...
      >
        <h1 className="text-2xl font-bold sm:text-3xl">{user.nickname}님의 주최한 모임</h1>
      </motion.div>
      ...
      {meetings.length > 0 ? (
        ...
        <div className="sinc-card p-16 text-center">
          ...
          <h3 className="mb-2 text-xl font-medium">주최한 모임이 없습니다</h3>
          <p className="text-muted-foreground">직접 모임을 만들어보세요!</p>
        </div>
      )}
    </div>
  );
